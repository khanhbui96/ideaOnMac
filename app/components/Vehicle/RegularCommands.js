...
function RegularCommands(props) {
  ...
  const newCommands = commands.data.filter(command => {
    return (
        command.license === number && command.time.slice(5,7).indexOf(time) !== -1
    );
  })
  const value = newCommands.length;
  const sum  = ()=>{
    return newCommands.reduce((a, b)=>{
      return {result: parseInt(a.result) + parseInt(b.result)}
    }, {result: 0})
  };
  useEffect(() => {
    getAllCommand();
